                    ...
                    <div class="cnt-body">
                        ...
                        <div class="post-tweet-container">
                            <div class="post-tweet">
                                <img class="p-photo"
                                    src="{{ auth()->user()->profile->pphoto ? asset('storage/' . auth()->user()->profile->pphoto) : asset('images/default.png') }}"
                                    alt="">
                                <div class="textarea-forpost">
                                    <form action="/" method="POST" enctype="multipart/form-data">
                                        @csrf

                                        <textarea class="inp" cols="50" rows="2" name="tweets" id="oop" placeholder="What's happening?"></textarea>
                                        ...
                                        <div class="upload">
                                            <label for="image" title="Upload Photo" style="cursor: pointer">
                                                <img src="/icon/image.svg" alt="">
                                            </label>
                                            <input type="file" name="image" id="image" style="display: none;">
                                            @error('image')
                                                <p style="font-size: small; color:red">{{ $message }}</p>
                                            @enderror
                                            <button class="send-tweet" type="submit">Post</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
                                        ...
                                        <a title="{{ $tweet->user->username . ' tweet' }}" href="/tweet/{{ $tweet->id }}"
                                            ...
                                            <div class="view-tweet">
                                                <div class="info">
                                                    <div class="prof">
                                                        <img class="p-photo"
                                                            src="{{ $tweet->user->profile->pphoto ? asset('storage/' . $tweet->user->profile->pphoto) : asset('storage/profilePhoto/default.jpeg') }}"
                                                            alt="">
                                                        <div class="id">
                                            ...
                                            @if ($tweet->image)
                                                <div class="image-post">
                                                    <img class="img-upload" src="{{ asset('storage/' . $tweet->image) }}"
                                                        alt="">
                                                </div>
                                            @endif
                                            <ul class="reactions">
                                                <li><img class="l-react" src="/icon/chat.svg"
                                                        alt="">{{ count($tweet->comments) }}</li>
                                                <li><img class="l-react" src="/icon/bookmark.svg"
                                                        alt="">{{ count($tweet->bookmarks) }}</li>
                                                <li><img class="l-react" src="/icon/heart.svg"
                                                        alt="">{{ isset($tweet->likes) ? count($tweet->likes) : 0 }}
                                                </li>
                                            </ul>
                                        </a>
                                    @endforeach
                                    ...
                                    <a href="/tweet/{{ $tweet['id'] }}" id="content"
                                        ...
                                        <ul class="reactions">
                                            <li> <img class="l-react" src="icon/chat.svg"
                                                    alt="">{{ count($tweet->comments) }}</li>
                                            <li> <img class="l-react" src="/icon/bookmark.svg"
                                                    alt="">{{ count($tweet->bookmarks) }}
                                            </li>
                                            <li> <img class="l-react" src="icon/heart.svg"
                                                    alt="">{{ isset($tweet->likes) ? count($tweet->likes) : 0 }}</li>
                                        </ul>
                                    </a>
                                @endforeach
                            @else
                                <h1 style="display: flex; justify-content:center; align-items:center; margin-top:20px;">Nothing
                                    Found</h1>
                            @endunless
                        </div>
                    </div>
